...
if (!function_exists('weaverx_setup')) :
    ...
    function weaverx_setup(): void
    {
        ...
        */
        ...
        weaverx_add_classic_editor_styles();
...
if (!function_exists('weaverx_init_opts')) :
    function weaverx_init_opts($who = ''): void
    {
        ...
        if (current_user_can('edit_theme_options') &&           // CSS cache bug - fixed 6.2.1.2, 6.2.1.3
            weaverx_getopt_default('theme_css_vers','') == ''
            && weaverx_getopt_default('wvrx_css_saved', '') != '') { // for now, just see if empty. May need to check version later on
            // OK, the weaverx_setopt('wvrx_css_saved', $current_css) needs to be cleared
            weaverx_setopt('wvrx_css_saved','');    // nothing saved
            weaverx_setopt('theme_css_vers', WEAVERX_VERSION); // set to this version. Could be used in a version_compare later if needed
            weaverx_save_opts(); // need to save this flag
            // don't display this any more... weaverx_alert(esc_html__('Norw: CSS cache cleared, new Theme version.', 'weaver-xtreme'));
        }
...
if (!function_exists('weaverx_enqueue_styles_action')) {
    ...
    function weaverx_enqueue_styles_action(): void
    {
        ...
        // This might be on-the-fly, or could have been generated already
        $current_css = weaverx_getopt('wvrx_css_saved');    // added V6.2.1.6 for the &gt; settings unsolved problem

        if (weaverx_getopt_checked('_disable_gen_css_cache') || empty($current_css) || is_customize_preview() || current_user_can('edit_theme_options')) {
            // we will always generate this on-the-fly if admin logged in
            // enqueue inline style - generate CSS rules in $GLOBALS['weaverx_gen_css'], then wp_add_line_style it.
        ...
        wp_add_inline_style('weaverx-style-sheet',$current_css);
    }
}
...
{    // action definition
    ...
    $useSM = weaverx_getopt_checked('use_smartmenus') ? '1' : '0';
